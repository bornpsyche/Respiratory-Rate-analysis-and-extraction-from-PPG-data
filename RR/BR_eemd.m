...
for k = 1:numSegments
    ...
    if isempty(s)
        ...
        X = fftshift(fft(s(:,1)));       
        [maxpeak, maxpeakindes] = max(abs(X)*2);
        results(j).eemd(1,k)=abs(f(maxpeakindes));
    end
    ...
    results(j).eemd(2,k)=abs(f(refmaxpeakindes));
    ...
    clear s p X refmaxpeak refmaxpeakindes;
     ...
     X = fftshift(fft(s(:,1)));       
     [maxpeak, maxpeakindes] = max(abs(X)*2);
     results(j).eemd(3,k)=abs(f(maxpeakindes));
    else
        results(j).eemd(3,k)=0;
    end
end
end
