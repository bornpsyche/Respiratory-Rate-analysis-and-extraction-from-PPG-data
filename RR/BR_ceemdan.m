...
for k = 1:numSegments
   ...
   % M( :, ~any(M,1) ) = [];  %columns
    ...
    if exist('M','var')
        ...
        results(j).eemd(1,k)=0;
    end
    
    if isempty(s)
        results(j).ceemdan(1,k)=0;
    else
        %s(:,1)=s(:,1)-mean(s(:,1));
        X = fftshift(fft(s(:,1)));       
        [maxpeak, maxpeakindes] = max(abs(X)*2);
        results(j).ceemdan(1,k)=abs(f(maxpeakindes));
    end
    ...
    results(j).ceemdan(2,k)=abs(f(refmaxpeakindes));
    ...
    clear s p X refmaxpeak refmaxpeakindes;
     ...
     X = fftshift(fft(s(:,1)));       
     [maxpeak, maxpeakindes] = max(abs(X)*2);
     results(j).ceemdan(3,k)=abs(f(maxpeakindes));
    else
        results(j).ceemdan(3,k)=0;
    end
end
end
