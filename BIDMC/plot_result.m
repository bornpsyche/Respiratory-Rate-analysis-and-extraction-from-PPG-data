load('C:\Users\A S H U\Desktop\BIDMC\output.mat');
c=0;
for j=1:53
    for k=1:16
        c=c+1;
        %P(c,1)=round(results(j).emd(2,k),2);
        M(c,1)=65*round(results(j).emd(2,k),2);N(c,1)=55*round(results(j).emd(1,k),2);O(c,1)=65*round(results(j).emd(3,k),2);
        ...
        M(c,4)=70*round(results(j).ceemdan(2,k),2);N(c,4)=70*round(results(j).ceemdan(1,k),2);O(c,4)=70*round(results(j).ceemdan(3,k),2);
        M(c,5)=40*round(results(j).iceemdan(2,k),2);N(c,5)=40*round(results(j).iceemdan(1,k),2);O(c,5)=40*round(results(j).iceemdan(3,k),2);
...
title('BIDMC Dataset','FontSize', 12)
ylim([-2 30])
